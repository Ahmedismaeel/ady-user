...
class CartRepository implements CartRepositoryInterface<ApiResponse> {
  ...
  Future<ApiResponse> addToCartListData(
    ...
    try {
      final response =
          await DioClient.instance.post(AppConstants.addToCartUri, data: data);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> updateQuantity(int? key, int quantity) async {
    try {
      final response = await DioClient.instance
          .post(AppConstants.updateCartQuantityUri, data: {
        '_method': 'put',
        'key': key,
        'quantity': quantity,
        'guest_id': Provider.of<AuthController>(Get.context!, listen: false)
  ...
  Future<ApiResponse> delete(int? key) async {
    try {
      final response =
          await DioClient.instance.post(AppConstants.removeFromCartUri, data: {
        '_method': 'delete',
        'guest_id': Provider.of<AuthController>(Get.context!, listen: false)
            .getGuestToken(),
        'key': key
  ...
  Future<ApiResponse> addRemoveCartSelectedItem(
    ...
    try {
      final response = await DioClient.instance
          .post(AppConstants.selectCartItemsUri, data: data);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
