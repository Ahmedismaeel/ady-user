...
class CheckoutRepository implements CheckoutRepositoryInterface {
  ...
  Future<ApiResponse> cashOnDeliveryPlaceOrder(
    ...
    try {
      final response = await DioClient.instance.get(
          '${AppConstants.orderPlaceUri}?address_id=$addressID&coupon_code=$couponCode&payment_method=$paymentMethod&coupon_discount=$couponDiscountAmount&billing_address_id=$billingAddressId&order_note=$orderNote&guest_id=${Provider.of<AuthController>(Get.context!, listen: false).getGuestToken()}&is_guest=${Provider.of<AuthController>(Get.context!, listen: false).isLoggedIn() ? 0 : 1}&is_check_create_account=$isCheckAccount&password=$password');
  ...
  Future<ApiResponse> offlinePaymentPlaceOrder(
    ...
    try {
      ...
      Response response = await DioClient.instance
          .post(AppConstants.offlinePayment, data: fields);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> digitalPaymentPlaceOrder(
    ...
    try {
      ...
      final response =
          await DioClient.instance.post(AppConstants.digitalPayment, data: {
        "order_note": orderNote,
        "customer_id": customerId,
        "address_id": addressId,
        "billing_address_id": billingAddressId,
