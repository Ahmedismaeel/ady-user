...
class PriceConverter {
  static String convertPrice(BuildContext context, double? price,
    ...
    return '${(singleCurrency ? price : (price ?? 0) * (Provider.of<SplashController>(context, listen: false).myCurrency?.exchangeRate ?? 0.00) * (1 / (Provider.of<SplashController>(context, listen: false).usdCurrency?.exchangeRate ?? 0.00)))?.toStringAsFixed(Provider.of<SplashController>(context, listen: false).configModel!.decimalPointSettings ?? 1).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
