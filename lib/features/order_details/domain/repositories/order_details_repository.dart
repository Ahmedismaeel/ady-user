...
class OrderDetailsRepository implements OrderDetailsRepositoryInterface {
  ...
  Future<ApiResponse> get(String orderID) async {
    try {
      final response =
          await DioClient.instance.get(AppConstants.orderDetailsUri + orderID);
  ...
  Future<ApiResponse> trackYourOrder(String orderId, String phoneNumber) async {
    try {
      final response = await DioClient.instance.post(AppConstants.orderTrack,
  ...
  Future<ApiResponse> reorder(String orderId) async {
    try {
      final response =
          await DioClient.instance.post(AppConstants.reorder, data: {
        'order_id': orderId,
      });
      return ApiResponse.withSuccess(response);
    } catch (e) {
  ...
  Future getOrderInvoice(String orderID) async {
    try {
      final response = await DioClient.instance
          .get('${AppConstants.generateInvoice}$orderID');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
