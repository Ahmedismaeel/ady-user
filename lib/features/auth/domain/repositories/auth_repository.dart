...
class AuthRepository implements AuthRepoInterface {
  ...
  Future<ApiResponse> socialLogin(Map<String, dynamic> socialLogin) async {
    try {
      Response response = await DioClient.instance
          .post(AppConstants.socialLoginUri, data: socialLogin);
  ...
  Future<ApiResponse> registration(Map<String, dynamic> register) async {
    try {
      Response response = await DioClient.instance
          .post(AppConstants.registrationUri, data: register);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> logout() async {
    try {
      Response response = await DioClient.instance.get(AppConstants.logOut);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<void> saveUserToken(String token) async {
    //DioClient.instance.updateHeader();
    // DioClient.instance.updateHeader(token, null); // Check
    try {
      await SharedPreferenceHelper.instance.saveToken(token);
  ...
  Future<ApiResponse> setLanguageCode(String languageCode) async {
    try {
      final response = await DioClient.instance.post(
          AppConstants.setCurrentLanguage,
  ...
  @override
  String getUserToken() {
    return SharedPreferenceHelper.instance.getToken() ?? "";
  }
  ...
  bool isLoggedIn() {
    return SharedPreferenceHelper.sharedPreference
        .containsKey(AppConstants.userLoginToken);
  }

  @override
  Future<bool> clearSharedData() async {
    // await sharedPreferences?.remove(AppConstants.userLoginToken);
    await SharedPreferenceHelper.instance.removeToken();
  ...
  Future<ApiResponse> resendEmailOtp(
    ...
    try {
      Response response = await DioClient.instance.post(
          AppConstants.resendEmailOtpUri,
          data: {"email": email, "temporary_token": temporaryToken});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponse> resetPassword(String identity, String otp,
    ...
    try {
      Response response =
          await DioClient.instance.post(AppConstants.resetPasswordUri, data: {
        "_method": "put",
  ...
  Future<ApiResponse> getGuestId() async {
    try {
      final response = await DioClient.instance.get(AppConstants.getGuestIdUri);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
