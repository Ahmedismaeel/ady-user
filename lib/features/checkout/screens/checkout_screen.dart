...
import 'package:flutter_sixvalley_ecommerce/features/checkout/screens/order_payment_screen.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
    ...
    return Scaffold(
      ...
      bottomNavigationBar:
        ...
        return Consumer<CheckoutController>(
            builder: (_c, orderProvider, child) {
          return Consumer<CouponController>(builder: (_c, couponProvider, _) {
            return Consumer<CartController>(builder: (_c, cartProvider, _) {
              return Consumer<ProfileController>(
                  builder: (_c, profileProvider, _) {
                            ...
                            if (orderProvider.addressIndex == null &&
                              ...
                              showCustomSnackBar(
                                  ...
                                  isToaster: true);
                                    ...
                                    -1 &&
                                orderProvider.codChecked == false)) {
                              // Navigator.of(context).push(MaterialPageRoute(
                              //     builder: (BuildContext context) =>
                              //         const SavedBillingAddressListScreen()));
                              showCustomSnackBar(
                                  getTranslated(
                                      'Please select payment method', context),
                                  context,
                                  isToaster: true);
                            }
  ...
  void _callback(BuildContext context, bool isSuccess, String message,
      int orderID, String paymentMethod, bool createAccount) async {
    if (isSuccess) {
      if (paymentMethod == "cash_on_delivery") {
        ...
        try {
          ...
          "${AppConstants.orderPayment}$orderID".log();
          PaymentResponse payment = await "${AppConstants.orderPayment}$orderID"
              .get(fromJson: PaymentResponse.fromJson);
          SmartDialog.dismiss();
          final paymentResponse = await SmartDialog.show(
              animationType: SmartAnimationType.fade,
              builder: (BuildContext context) => PayOrderView(
                    url: payment.payUrl ?? "",
                    orderId: orderID,
                  ));
          // await Navigator.of(context).push(
          //   MaterialPageRoute(
          //       builder: (_) => PayOrderView(
          //             url: payment.payUrl ?? "",
          //           )),
          // );
          if (paymentResponse) {
