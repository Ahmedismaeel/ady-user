...
class ChatController extends ChangeNotifier {
  ...
  ChatModel? customerModel;
  ChatModel? aderModel;

  ChatModel? searchChatModel;
  ChatModel? searchAderModel;
    ...
    if (apiResponse.response != null &&
      ...
      if (offset == 1) {
        if (userType == 0) {
          ...
          chatModel = null;
          aderModel = ChatModel.fromJson(apiResponse.response!.data);
        }
      } else {
        if (userType == 0) {
  ...
  Future<void> searchChat(
    ...
    if (apiResponse.response != null &&
      ...
      if (userIndex == 0) {
        ...
        searchAderModel = null;
        searchAderModel =
            ChatModel(totalSize: 1, limit: '10', offset: '1', chat: []);

        apiResponse.response!.data
            .forEach((chat) => searchAderModel!.chat!.add(Chat.fromJson(chat)));
        searchAderModel?.chat = searchAderModel!.chat;
      }
    } else {
      _isLoading = false;
      ApiChecker.checkApi(apiResponse);
